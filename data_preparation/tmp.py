...
for i, sentence in enumerate(test.text):
	...
	for j, item in enumerate(preds[str(i+1)]):
		if item != 'O':
			...
			index = sentence.lower().index(test_data[i][1][j])
...
spans_pred.close()
	# print(len(clean_words), len(preds[str(i+1)]), len(test_data[i][1])-1)
	# print(preds[str(i+1)])
	# for j, item in enumerate(preds[str(i+1)]):
	# 	if item == 'O':
	# 		pivot = pivot + len(words[j])
	# 	if item == 'B-LOC' or item == 'I-LOC':
	# 		span = range(pivot, len(words[j]))
